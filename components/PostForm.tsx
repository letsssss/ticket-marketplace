...
export function PostForm({ onCancel }: PostFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (content.trim().length < 10) {
      toast.error('내용은 10글자 이상 입력해주세요')
      return
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label htmlFor="content" className="block text-sm font-medium text-gray-700 mb-1">
          내용 <span className="text-xs text-red-500">(10글자 이상 입력해주세요)</span>
        </label>
        <Textarea
          id="content"
          value={content}
          onChange={(e) => setContent(e.target.value)}
          placeholder="내용을 입력하세요 (최소 10글자 이상)"
          className="min-h-[200px]"
          required
        />
        <p className="text-xs text-gray-500 mt-1">
          {content.length}/10 글자 (최소 10글자 이상 입력해야 합니다)
        </p>
      </div>
